...
real(kind=8)    :: xp,yp,zp,rp,gp,wp,S0(0:1,2),veloc(3), &
                   curr_p(0:1,0:1)
...
use omp_lib
implicit none
integer, intent(in)           :: np,nx,nr,nkO,nchnk
integer, intent(in)           :: IndInChunk(0:nchnk),guards
...
!$omp parallel default(shared) &
!$omp private(loc_left,loc_right,xp,yp,zp,rp,wp,S0,dens_p, &
!$omp         phaseO,phase_m,ix,ir,ip,k,iO,ichnk,nxleft)
