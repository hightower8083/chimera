subroutine dep_curr_chnk(coord,coord_cntr,curr,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv,&
                    np,nx,nr,nkO,nchnk)
use omp_lib
implicit none
...
!f2py intent(in) :: coord,coord_cntr,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv
...
!$omp private(loc_left,loc_right,xp,yp,zp,rp,wp,S0,veloc,curr_p,phaseO,phase_m,ix,ir,ip,k,l,iO,ichnk,nxleft)
...
subroutine dep_dens_chnk(coord,dens,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv,&
                    np,nx,nr,nkO,nchnk)
use omp_lib
implicit none
integer, intent(in)        :: np,nx,nr,nkO,nchnk,IndInChunk(0:nchnk),guards
...
complex(kind=8) :: ii=(0.0d0,1.0d0),phaseO(0:nkO),phase_m
complex(kind=8), allocatable :: loc_left(:,:,:), loc_right(:,:,:)
!f2py intent(in) :: coord,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv
...
subroutine dep_dens_cntr_chnk(coord,wght,dens,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv,&
                    np,nx,nr,nkO,nchnk)
use omp_lib
implicit none
integer, intent(in)        :: np,nx,nr,nkO,nchnk,IndInChunk(0:nchnk),guards
...
complex(kind=8) :: ii=(0.0d0,1.0d0),phaseO(0:nkO),phase_m
complex(kind=8), allocatable :: loc_left(:,:,:), loc_right(:,:,:)
!f2py intent(in) :: coord,wght,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv
!f2py intent(in,out) :: dens
!f2py intent(hide) :: np,nx,nr,nkO,nchnk
...
subroutine dep_curr_env_chnk(coord,coord_cntr,curr,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv,&
...
real(kind=8)    :: xp,yp,zp,rp,S0(0:1,2),veloc(3)
complex(kind=8) :: ii=(0.0d0,1.0d0),curr_p(0:1,0:1),phaseO(0:nkO),phase_m,wp
complex(kind=8), allocatable :: loc_left(:,:,:,:),loc_right(:,:,:,:)

!f2py intent(in) :: coord,coord_cntr,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv,kx0
!f2py intent(in,out) :: curr
!f2py intent(hide) :: np,nx,nr,nkO,nchnk
...
subroutine dep_dens_env_chnk(coord,dens,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv,&
                    kx0,np,nx,nr,nkO,nchnk)
use omp_lib
implicit none
integer, intent(in)        :: np,nx,nr,nkO,nchnk,IndInChunk(0:nchnk),guards
...
!f2py intent(in) :: coord,IndInChunk,guards,leftX,Rgrid,dx_inv,dr_inv,kx0
!f2py intent(in,out) :: dens
!f2py intent(hide) :: np,nx,nr,nkO,nchnk
...
if (Rgrid(0)<0) then
  ...
  if (nkO>0) then
    ...
    dens(:,1,-nkO:-1) = dens(:,2,-nkO:-1)/9.0d0
  endif
  dens(:,0,:) = 0.0
endif
