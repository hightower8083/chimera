...
class SR:
	def __init__(self,sr_in):
		...
		self.Data['omega'] = self.Args['omega']
		...
		if self.Args['Mode'] == 'far':
			...
			self.Data['theta'] = self.Args['theta']
			self.Data['phi'] = self.Args['phi']
			...
			self.Data['X'] = self.Args['X']
			self.Data['Y'] = self.Args['Y']
			self.Data['Z'] = self.Args['Z']
	...
	def init_track(self,Steps,beam):
		...
		self.Data['weights'] = np.zeros((Steps,Nparts),order='F')
		self.Data['weights'][0,:] = beam.Data['weights']
	...
	def damp_track(self,out_folder='./'):
		...
		if self.Args['Mode'] == 'far':
			comps = ['coords','momenta_prv','momenta_nxt','weights']
		elif self.Args['Mode'] == 'near' or self.Args['Mode'] == 'near-circ':
			comps = ['coords','momenta','weights']
